...
useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/students/${rollNumber}/${academicYear}`);
      setStudent(response.data);
      console.log(response.data);
      setGender2(response.data.gender === "m" ? "Mr." : "Ms.");
      if (response.data.bonafied_provided>1) {
        setBp(1);
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <div style={{ textAlign: "center"}}>
       {bp === 1 && <p style={{ color: "red", fontWeight: "bold" }}>Bonafide already provided</p>}
