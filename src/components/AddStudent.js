...
const MemoizedLink = React.memo(Link);
  ...
  const update = async (event) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/students/year/${year2}`, student);
  ...
  const failedupdate = async (event) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/students/failyear/${failedrollno}`, student);
      console.log(response.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const updateacademicyear = async (event) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/students/academic_year/${academicyear2}`, student);
      console.log(response.data);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    ...
    <div  >
      ...
      <h2 className="heading_login_box" style={{ borderBottom: "2px dotted black" }}>Designed and developed by Mr.Talha M. Chinchali (Dept. of CSE)</h2>{addis ? (
      ...
      {searchis ? (
        <div className='login_box' style={{ position: "relative", left: "1000px", top: "300px" }}>
          ...
          <div className='user_box'>
            ...
            <input className="user_box_input" placeholder='academic year' type="text" value={academicYear} onChange={(event) => setacademicYear(event.target.value.toUpperCase())} />
          </div>
          <MemoizedLink to={`/students/${rollNumber2}/${academicYear}`}>
  <button className="login_box_button" type="submit">Bonafied</button>
</MemoizedLink>
          <Link to={`/ssp/${rollNumber2}/${academicYear}`}>
